...
function proxy(req, options, res) {
  ...
  return new Promise((resolve, reject) => {
    ...
    agentReq.once('response', (agentRes) => {
      ...
      // data unziped
      const unzipStream = unzipHttpStream(agentRes);
      agentRes.contentEncoding = null;

      readBody(unzipStream, parameters.charset, function(text) {
        ...
        if (mime.isJSON(type)) {
          try {
            ...
            _resReject(new ProxyRequestError('proxy parse body to json fail', err), {resBody: text});
          }
        }
