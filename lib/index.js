...
function proxy(req, options, res) {
  ...
  if (options.bypassModifyResponse && typeof options.bypassModifyResponse !== 'function') {
    throw new ProxyRequestError('options.bypassModifyResponse must be a function');
  }
  ...
  const bypassModifyResponse = options.bypassModifyResponse;
  const modifyResponse = options.modifyResponse;
  const onResponse = options.onResponse;
  delete options.bypassModifyResponse;
  ...
  return new Promise((resolve, reject) => {
    ...
    function _onResponse(agentRes) {
      ...
      function _resReject(err, extra) {
        const resCacheStream = agentRes.pipe(new CacheStream());
      ...
      function _resolve(resp) {
        ...
        if (resp instanceof CacheStream) {
          resCacheStream = resp;
        } else {
      ...
      try {
        onResponse && onResponse(agentRes);
      } catch (err) {
      ...
      if (!modifyResponse
        || agentRes.isNoBody
        || (bypassModifyResponse && bypassModifyResponse(agentRes))
      ) {
        _resolve(agentRes);
        return;
      }
      ...
      rc.readBody(unzipStream, charset)
        .then(text => {
        ...
        .catch(err => _resReject(new ProxyRequestError('proxy unzip error', err)));
    }
  });
}
